...
module.exports = {
    ...
    getFoodById: async (req,res) => {
        ...
        try {
            ...
            if(!food) {
                return res.status(400).json({status: false, message: "Food not found"});
            }
    ...
    getFoodByRestaurant: async (req, res) => {
        ...
        try {
            ...
            if(!foods || foods.length === 0) {
                res.status(404).json({status: false, message: "No food items found"})
            }
    ...
    foodAvailabiltiy: async (req, res) => {
        const foodId = req.params.id;
    ...
    updateFoodById: async(req, res) => {
        ...
        try {
            ...
            if(!updatedFood) {
                res.status(404).json({status: false, message: "Food item not updated"})
            }
    ...
    addFoodTag: async(req,res) => {
        ...
        try {
            ...
            if(food.foodTags.includes(tag)) {
                res.status(400).json({status: false, message: "Tag already exist"})
            }
    ...
    addFoodType: async(req,res) => {
        ...
        try {
            ...
            if(food.foodType.includes(foodType)) {
                res.status(400).json({status: false, message: "Food type already exist"})
            }
